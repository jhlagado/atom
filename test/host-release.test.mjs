...
test("the product documentation, release gate, license, and measured account agree", async () => {
  ...
  const phase9 = JSON.parse(await fs.readFile("proofs/phase-9.json", "utf8"));
  assert.equal(phase9.native.codeAndTables, native.codeBytes);
  assert.equal(phase9.native.linkedResidentExtent, native.residentExtentBytes);
  assert.equal(phase9.native.physicalMarginBelow16KiB, 0x4000 - native.residentExtentBytes);
});
