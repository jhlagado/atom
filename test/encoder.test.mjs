...
test("recognizes every mnemonic through mixed case and rejects non-mnemonics", () => {
  ...
  for (const name of ["", "NOPE", "LDIRS", "A-B", "1234", "_LD"]) {
...
test("formLength ignores all concrete values", () => {
  for (const { source, record } of validCases().filter((_, index) => index % 31 === 0)) {
    const expected = harness.length(record);
    const changed = record.slice();
    changed.set([0x5a, 0xa5, 0xff, 0x00, 0x80, 0x7f], 4);
