...
to the same 12,381-byte resident image as the AZM build. That Atom-produced
...
The current native account is **Measured: 11,850 bytes** of code and immutable
tables. The linked resident image, including 531 bytes of fixed workspace, is
**Measured: 12,381 bytes**, leaving **Measured: 4,003
...
The generated form contains **Measured: 7,241 statements** from **Measured: 13
input files**. Its five code-bearing parts occupy **Measured: 101,631 bytes**.
The checked `%INCLUDE` entry adds one small masked part, for **Measured: 101,804
bytes across six parts** at the native boundary. The symbol map records
**Measured: 863 global names and 428 private names**. Atom itself performs no
...
All three produce **Measured: 12,381 identical bytes**, comprising **Measured:
12,008 initialized bytes and 373 reserved bytes**. The native stream applies
**Measured: 1,962 PATCH records** and reports **Measured: 1,291 declarations**.
The proof compares the whole resident extent, not a digest or a selected set of
instructions.

Both native generations execute **Measured: 99,279,516 Z80 instructions and
1,060,540,694 T-states**, with **Measured: 13,972 host service calls**. At 4
MHz, the cycle count corresponds to **Projected: at least 265.1
